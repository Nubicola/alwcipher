...
func makeInputArea(i *boundInput, o *boundOutput, c *corpus.Corpus, calcs *calculators) fyne.CanvasObject {
	...
	ui.calculateButton = widget.NewButton("Calculate", func() {
		...
		o.outputFieldBoundValue.Set(strings.Join(c.Get(val), "\n"))
	})
...
func MakeUI(c *corpus.Corpus) fyne.CanvasObject {
	...
	// write to corpus?
	writeToCorpusCheckbox := widget.NewCheck("Change corpus?", func(value bool) {
		// nothing
	})
	...
	content := container.NewBorder(jbutton, nil, nil, nil, box)
	return content
}
